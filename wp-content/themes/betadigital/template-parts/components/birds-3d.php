<div id="birds-3d-container" style="top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999;"></div>
...
<script type="module">
  ...
  loader.load(birdTexturePath, (texture) => {
      ...
      // Parâmetros de comportamento de bando
      ...
      const boundary = 70; // Limite da área de voo para X e Z (de -70 a 70)
      const minY = -5; // Limite inferior para Y
      const maxY = 20; // Limite superior para Y

      // NOVOS PARÂMETROS PARA LIMITES SUAVES
      const boundaryMargin = 15; // Distância da borda onde a força de repulsão começa a agir
      const boundaryForceStrength = 0.05; // Força da repulsão (ajuste conforme necessário)
      ...
      for (let i = 0; i < birdCount; i++) {
        ...
        mesh.position.set(
          ...
          (Math.random() - 0.5) * (maxY - minY) / 2 + (maxY + minY) / 2, // Centraliza Y entre minY e maxY
          (Math.random() - 0.5) * boundary
        );
      ...
      function animate() {
        ...
        birds.forEach((bird) => {
          ...
          // Adiciona a força de desvio de limite à direção do pássaro
          bird.userData.direction.add(boundaryAvoidance);
          bird.userData.direction.normalize(); // Re-normaliza a direção após adicionar a força de limite
          // --- FIM DA LÓGICA DE LIMITES SUAVES ---
