export default class WaveLines {
  ...
  createWaves() {
    ...
    // Range válido de left para linhas rotacionadas a -45°
    // (valores maiores que ~15% fazem o SVG sair pela direita)
    const LEFT_MIN   = -25;
    const LEFT_MAX   =  15;
    const rangeWidth = LEFT_MAX - LEFT_MIN;           // 40%
    const zoneWidth  = rangeWidth / colors.length;    // 8% por zona
    ...
    colors.forEach((color, index) => {
      ...
      // Centro da zona desta linha dentro do range válido
      const baseLeft = LEFT_MIN + index * zoneWidth + zoneWidth / 2;
      const drift    = zoneWidth * 0.4; // variação dentro da zona (±40% da zona)

      const initialTop = positions[index];
      const initialLeft = baseLeft + (-drift + Math.random() * drift * 2);
      const initialRotation = -45 + (-8 + Math.random() * 16); // -53 a -37 graus

      this.waves.push({
        svg,
        path,
        // Zona de ancoragem horizontal
        baseLeft,
        drift,
        // Amplitude
  ...
  animate() {
    this.waves.forEach((wave) => {
      ...
      if (wave.repositionTimer >= wave.repositionInterval) {
        ...
        // Deriva ancorada à zona horizontal desta linha
        wave.targetLeft = wave.baseLeft + (-wave.drift + Math.random() * wave.drift * 2);
        wave.targetRotation = -45 + (-8 + Math.random() * 16); // -53 a -37 graus

        // Limitar dentro dos bounds
        wave.targetTop = Math.max(5, Math.min(85, wave.targetTop));
        wave.targetLeft = Math.max(wave.baseLeft - wave.drift, Math.min(wave.baseLeft + wave.drift, wave.targetLeft));
